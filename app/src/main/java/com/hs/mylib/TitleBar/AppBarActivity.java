 ...
 * @author zhanghaitao
 */
public abstract class AppBarActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    /*//设置右边图片的样式
    public void setRightImage(int width,int height,int image) {
        if (mBarRightImage != null) {
            int visibility = mBarRightImage.getVisibility();
            if (visibility == View.GONE ||visibility==View.INVISIBLE) {
                mBarRightImage.setVisibility(View.VISIBLE);
            }
            *//*LinearLayout.LayoutParams layoutParams = (LinearLayout.LayoutParams) mBarRightImage.getLayoutParams();
            layoutParams.width = width;
            layoutParams.height = height;
            mBarRightImage.setLayoutParams(layoutParams);*//*
            mBarRightImage.setImageResource(image);
        }
    }*/
     ...
     * 在需要做保存的时候重写
     *
     * @return 决定是否执行finish操作
     */
    public abstract boolean savaData();
